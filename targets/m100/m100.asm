...
;
; Used by the routines at STEP, GETWORD, DMUL, __LOG, __SQR, __RND,
...
; __CLOAD, CLOADM, __CLEAR, GETVAR, SCPTLP, USING, NULOPN and __MAX.
SN_ERR:
  LD E,$02
...
;
; Used by the routines at DDIV, INT_DIV and INTEXP.
; a.k.a. OERR, DZERR
O_ERR:
  LD E,$0B
...
; OV error: overflow
;
; Used by the routines at GETWORD, DADD, DMUL, DDIV, __TAN, __EXP,
...
;
; Used by the routines at TO, EVAL3, INVSGN, VSIGN, __CINT, __CSNG, __CDBL,
; TSTSTR and __INT.
TM_ERR:
  LD E,$0D
...
;
; Used by the routines at CHRGTB, __CHRCKB, LNUM_PARM, __IF, DTSTR, FILIDX,
; __OPEN and __CLOSE.
;
...
__RESUME_0:
  ...
  CP TK_NEXT                  ;(TK_NEXT): RESUME NEXT?
  JP Z,RESNXT                 ;YUP.
  CALL ATOH                   ;GET FOLLOWING LINE #
...
ENDIF
  ...
  PUSH HL
  LD E,A
  LD HL,WORDS               ;GET PTR TO START OF RESERVED WORD LIST
DETOKEN_NEXT3:
  LD A,(HL)                 ;GET CHAR
  INC HL                    ;BUMP POINTER
  OR A                      ;SET CC'S
...
__EOF:
  ...
  JP Z,CF_ERR           ;BAD FILE NUMBER - NOT FOUND !!!
  CP $01                ; IS IT A SEQUENTIAL OUTPUT FILE?
  JP NZ,NM_ERR          ; NM error: bad file name
  PUSH HL
  CALL __EOF_1
  LD C,A
...
; This entry point is used by the routine at __MDM.
ONOFF_OPTIONS:
  CP TK_ON      ; TK_ON, token for 'ON' keyword
  JP Z,TIME_S_ON
  CP TK_OFF     ; TK_OFF
  JP Z,TIME_S_OFF
  CP TK_STOP    ; TK_STOP
...
; Routine at 7096
__KEY:
  CP TK_LIST        ; TK_LIST, "KEY LIST" command ?
  JP NZ,KEY_CONFIG
  RST CHRGTB        ; Gets next character (or token) from BASIC text.
  PUSH HL
  LD HL,FNKSTR
...
MOTOR_ON:
  RST SYNCHR
  DEFB TK_ON        ; TK_ON, token for 'ON' keyword
  DEC HL
  LD A,H
...
;
; Used by the routines at ERESET, READY, __LINE, __READ, CAS_OPNI_CO, IN_PRT,
...
; This entry point is used by the routine at DSUB.
DADD_0:
  AND $7F
  ...
  JP NC,NOSWAP_DEC      ;PUT THE SMALLER NUMBER IN FAC                ; No - Don't swap them
...
DADD_SWAP:
  ...
  DEC B                 ;ARE WE DONE?
  JP NZ,DADD_SWAP       ;NO, DO THE NEXT LO BYTE
...
NOSWAP_DEC:
  ...
  XOR A                 ; (GWBASIC: WANT TO ZERO OVERFLOW BYTES)
...
;*************************************************************
  ...
  OR $10
  LD (HL),A
  JP DECROU             ; Round it up        ; (GWBASIC: 'ROUND': GO ROUND THE RESULT)
...
; This entry point is used by the routines at DMUL and __RND.
...
DECNRM_0:
  ...
  OR A                ; Is it zero?
  JP NZ,DNORML        ; No - Do it bit at a time        ;NO, SHIFT ONE PLACE AT A TIME
...
DNORM5:
  ...
  JP Z,DECROU          ;NONE WAS, PROCEED TO ROUND THE NUMBER
  LD HL,FACCU          
...
_BCDADD:
  ...
  LD DE,FACCU+7
  LD B,$07
; This entry point is used by the routines at DMUL and DDIV.
DAA_PASS2:
  XOR A
...
DNORML_RLD_0:
  ...
  LD C,$04
RLDLOOP:
  PUSH HL              ;SAVE POINTER TO FP VALUE
  OR A
RLDLOOP_0:
  LD A,(HL)
  RLA
  ...
  DEC B                ;ARE WE DONE?
  JP NZ,RLDLOOP_0      ;NO, DO THE NEXT BYTE
  LD B,D
  POP HL
  DEC C
  JP NZ,RLDLOOP
...
; This entry point is used by the routines at DADD and PUFOUT.
DSHFRB:
  LD HL,FACCU+1
DSHFRB_LP:
...
;
    ...
    ;ALTERS ALL REGISTERS
;
; Used by the routines at __LOG, MULPHL, NEGAFT, FMULT and DEXP.
DMUL:
  RST TSTSGN         ;ZF=1 WILL BE SET IF (FAC)=0    ; Test sign of FP accumulator
...
;
; Used by the routines at BCDADD and DMUL.
LDIR_C:
  LD A,(HL)
  LD (DE),A
  DEC HL
...
; Routine at 12041
; Used by the routine at __TAN.
__SIN:                    ;WILL CALCULATE X=FAC/(2*PI)
  LD HL,FP_EPSILON        ;FP_EPSILON: 1/(2*PI) =~ 0.159155
  CALL MULPHL
...
__LOG:
  RST TSTSGN            ; test FP number sign
...
__LOG_0:
  ...
  CALL STAKFP
  CALL STAKFP
  LD HL,FP_LOGTAB_Q     ; 5 values series
...
__EXP:
  ...
  CALL STAKFP
  CALL __CINT         ;Determine integer power of 2.
  LD A,L
  RLA
  SBC A,A
  CP H
  JP Z,__EXP_1        ;JUMP IF NOT OVERFLOW
  LD A,H
...
; Move ARG to FAC1
;
; Used by the routines at __SQR, DBL_FACCU2HL, DEXP and INTEXP.
ARG_FAC1:
  LD HL,ARG

; Move (HL) to FAC1
;
; Used by the routines at __SIN, __ATN, __EXP, __RND, NEGAFT, PUFOUT, DEXP
; and INTEXP.
HL2FACCU:
  LD DE,FACCU
  JP HL2ARG_0
...
; Move FAC1 to (HL)
;
; Used by the routines at __EXP, __RND, NEGAFT, DEXP and INTEXP.
DBL_FACCU2HL:
  LD DE,FACCU
...
;
; Used by the routines at TSTSGN, VSIGN, FCOMP and ICOMP.
SIGN:
  LD A,(FACCU)           ; Get sign of FPREG
  OR A                                                      ;CHECK IF THE NUMBER IS ZERO
  RET Z                  ; RETurn if number is zero         ;IT IS, A IS ZERO
  LD A,(FACCU)           ; Get MSB of FPREG                           ;GET SIGN OF FAC, IT IS NON-ZERO
  JP ICOMPS              ; Possible optimization here:  CP 2Fh ..to hide the "CPL" instruction ?
...
; This entry point is used by the routines at DADD, DMUL, DDIV and
; __INT.
ZERO:
...
;
; This entry point is used by the routines at __COS, NEGAFT, HL_CSNG, __FIX, IMULT and FSUB.
NEG:
...
;
; Used by the routines at DSUB and __NEXT.
HL_ARG:
...
;
; Used by the routines at __IPL, ON_TIME_S, MAKTXT, __RND, DBL_FACCU2HL, FPTHL and
; BOOT.
MOVE1:
...
;VDFACS:
  ...
  CP $02            
  RET NZ            ;GO MOVE IT IF WE DO NOT HAVE A DBL
...
FCOMP:
  ...
  RST TSTSGN        ; Test sign of FPREG                ;CHECK IF FAC IS ZERO
...
; a.k.a. DCOMPD
; This entry point is used by the routines at MULPHL, DCOMP, INTEXP and
; __NEXT.
XDCOMP:
...
XDCOMP_0:           
  ...
  INC HL            ; ORDER BYTES
  DEC B             ;ARE WE DONE?
  JP NZ,XDCOMP_0    ;NO, COMPARE THE NEXT BYTES
...
CONIS1:
  POP DE
  JP MAKINT         ;STORE IT IN THE FAC AND SET VALTYP
...
__CDBL:
  ...
  CALL M,CONSI         ;CONVERT TO SNG IF WE HAVE AN INT
    ...
    ;ALTERS A,H,L
; This entry point is used by the routines at __LOG, FLOAT_ADD, FMULT, FDIV, FEXP and INTEXP.
CONDS:
  LD HL,$0000          ;ZERO H,L
...
__INT_0:
  ...
  JP M,__INT_2         ; IF NEGATIVE, USE SPECIAL FLOOR LOGIC
...
__INT_1:
  ...
  DEC B                ; DJNZ
  JP NZ,__INT_1        ; CONTINUE WITH LESS SIGNIFICANT BYTES
  RET
...
MLDEBC:
  ...
  JP Z,MLDEBC_2    ; Return zero if zero               ;THIS IS DONE FOR SPEED
...
MLDEBC_0:
  ...
  EX DE,HL                                             
  JP NC,MLDEBC_1   ; Bit was zero - No add             ;ADD IN (BC) IF HO WAS 1
...
IMULT_0:
  ...
  EX DE,HL
  JP NC,IMULT_1    ;DON'T ADD IN ANYTHING
  ADD HL,BC        ;ADD IN (BC)
  JP C,IMULT5      ;CHECK FOR OVERLFOW
IMULT_1:
  DEC A            ;ARE WE DONE?
  JP NZ,IMULT_0    ;NO, DO IT AGAIN
...
IMULT_3:
  ...
  JP IMULT5_0      ; (possible optimization: "LD BC,nn" OVER NEXT 2 BYTES)
...
;
    ...
    ;ALTERS A,B,C,D,E,H,L
;
; Used by the routines at _GETYPR and INEG.
INT_DIV:
  LD A,H
...
; This entry point is used by the routines at HL_CSNG, TSTSTR, ISUB, INEG
; and INTEXP.
INEGHL:
  XOR A             ;CLEAR A
...
;
; (this entry is not used by the core ROM)
FADDS:
...
_ASCTFP_0:
  RST CHRGTB                                             ;HERE TO CHECK FOR A DIGIT, A DECIMAL POINT, "E" OR "D"
  JP C,_ASCTFP_DIGITS  ; Set result to zero              ;GET THE NEXT CHARACTER OF THE NUMBER
  CP '.'               ; Digit - Add to number           ;WE HAVE A DIGIT
  JP Z,DPOINT                                            ;CHECK FOR A DECIMAL POINT
  CP 'e'               ; "." - Flag point                ;WE HAVE ONE, I GUESS
  JP Z,EXPONENTIAL                                       ;LOWER CASE "E"
...
EXPONENTIAL:           
  ...
  JP Z,_ASCTFP_2       ;IF SO POSSIBLE ELSE
  CP 'L'               ;IS THIS REALLY AN "ELSE"?
  JP Z,_ASCTFP_2       ;WAS ELSE
  CP 'q'               ;SEE IF LOWER CASE "Q"
  JP Z,_ASCTFP_2       ;IF SO POSSIBLE "EQV"
  CP 'Q'               ;POSSIBLE "EQV"
_ASCTFP_2:
  POP HL               ;RESTORE [H,L]
  JP Z,_ASCTFP_3       ;IT WAS JUMP!
...
ADDIG:
  ...
  RST CPDEHL        ;COMPAR RETURNS WITH CARRY ON IF            ; Compare HL with DE.
...
FINDG_BCD:
  ...
  RRA
  LD A,C
  JP NC,FINDGE1     ; HI or LOW BCD digit ?
  ADD A,A
  ADD A,A
  ADD A,A
  ADD A,A
...
FOUT:
  XOR A               ;ENTRY TO PRINT THE FAC IN FREE FORMAT
                      ;SET FORMAT FLAGS TO FREE FORMATTED OUTPUT
...
; This entry point is used by the routine at LINPRT.
SPCFST:
...
FOUFRV:
  ...
  JP C,FOFRS1         ;IT SHOULD, IT IS .LT. .01
...
;;;DEFB $01                ; "LD BC,nn" OVER THE NEXT 2 BYTES   <- Possible optimization
  JP FOUBE3_0           ; "LD BC,nn" OVER THE NEXT 2 BYTES
...
FOUFXV:
  ...
  JP C,FFXSDC           ;IF THE FAC WAS SMALL ENOUGH, GO PRINT IT
...
FFXFLV:
  ...
  RST TSTSGN           ;SEE IF WE HAVE ZERO
  PUSH AF
...
FFXLV4:                ; SO IGNORE IT.
  ...
  LD A,(FACCU)
  JP Z,FFXLV2          ;EXPONENT=0 IF THE NUMBER IS ZERO
...
; ROUTINE TO PUT IN DECIMAL POINT AND LEADING ZEROS
...
FTN10:              
  ...
  LD C,B            
  JP NZ,FTN10       ;PUT IN LEADING ZEROS UNTIL B ZERO
...
;*************************************************************
;
;       $FOUTCI  CONVERT THE INTEGER IN (FACLO)-TWO BYTES TO ASCII DIGITS.
;       $FOUTO and $FOUTH  do not exist here
;
;**************************************************************
...
__OPEN_0:
  RST SYNCHR
  DEFB TK_FOR
  CP TK_INPUT           ; TK_INPUT, 'INPUT' TOKEN code
  LD E,$01
  JP Z,__OPEN_INPUT
  
  CP TK_OUT             ; TK_OUT ('OUT..P.U.T' !)
  JP Z,__OPEN_OUTPUT
...
__MERGE_0:
  ...
  JP Z,__MERGE_2     ;NO, JUST LOAD
  LD A,(HL)
  SUB ','            ;GOTTA HAVE A COMMA
  OR A
  JP NZ,__MERGE_2    ;NO, JUST LOAD
  RST CHRGTB        ; Gets next character (or token) from BASIC text.
  RST SYNCHR
...
__CLOSE:
  ...
  JP NZ,__CLOSE_1       ;NOT END OF STATEMENT, SO SCAN ARGUMENTS
  PUSH HL               ;SAVE THE TEXT POINTER
__CLOSE_0:
  PUSH AF
...
__CLOSE_1:
  ...
  PUSH HL               ;SAVE THE TEXT POINTER ON THE STACK
IF M100
  SCF
ENDIF
  CALL CLSFIL
...
;
; Used by the routines at RAM_OPN and NULOPN.
AO_ERR:
  LD E,$35
...
TXT_CTL_I:
  PUSH AF
  CALL TXT_CTL_C
  CALL TXT_SET_FIRSTROWPTR
  CALL TXT_GET_CURPOS
  POP AF
; This entry point is used by the routine at TXT_CTL_M.
TXT_CTL_I_0:
  CALL _SPLIT_ROW
  JP C,TXT_MEMFULL      ; JP if not at the end of text
  PUSH HL
  CALL _INC_ROWPTR
  POP HL
  JP _UPDATE_TOPROWPTR
...
TXT_SHOW_SELECTION:
  ...
  POP HL
  RST CPDEHL
  JP C,TXT_SHOW_SELECTION_0
  CALL GO_BOTTOMROW
TXT_SHOW_SELECTION_0:
  CALL C,TXT_LOCATE_PTR
  LD H,L
  EX (SP),HL
  CALL TXT_CKRANGE
  JP NC,TXT_SHOW_SELECTION_1
  LD L,$01
TXT_SHOW_SELECTION_1:
  CALL NC,TXT_LOCATE_PTR
  POP AF
  SUB L
  LD C,A
  EX DE,HL
  ...
  CALL TXT_GET_CURROWPTR
  LD A,C
TXT_SHOW_SELECTION_2:
  PUSH AF
  CALL TXT_GET_ROWPTR
  POP AF
  DEC A
  JP P,TXT_SHOW_SELECTION_2
  JP _POSIT
...
TXT_ENSURE_VISIBLE:
  CALL TXT_CKRANGE
  CALL C,TXT_SCROLL_PREVROW_0
  JP C,TXT_ENSURE_VISIBLE
TXT_ENSURE_VISIBLE_0:
  PUSH HL
  CALL TXT_GET_BOTTOMROWPTR
  POP HL
  RST CPDEHL
  CALL NC,TXT_SCROLL_NEXTROW_0
  JP NC,TXT_ENSURE_VISIBLE_0
...
TXT_SCROLL_PREVROW:
  INC L
TXT_SCROLL_PREVROW_0:
  PUSH AF
  PUSH HL
  CALL CLR_BOTTOMROW
  CALL HOME
  CALL INSLIN
  CALL TXT_GET_PREVROWPTR
  PUSH DE
  CALL TXT_GET_BOTTOMROWPTR
  INC HL
  LD E,L
...
TXT_CHGET:
  ...
  LD A,(LABEL_LN)
  CP B
  JP NZ,TXT_CHGET_RFSH
  POP AF
  RET

TXT_CHGET_RFSH:
  AND A
  JP Z,TXT_CHGET_REBUILD
  LD A,(CSRX)
  CP L
  LD A,(ACTV_X)
  CALL NZ,TXT_DELETE_ROWTAB
  POP AF
  RET
...
TXT_PASTE_0:
  INC DE
  LD A,(DE)
  CP $1A        ; EOF:  find EOF in COPY/PASTE buffer
  JP NZ,TXT_PASTE_0
  CALL TXT_COPY_0
  PUSH AF
  PUSH DE
...
TXT_CTL_V_0:
  ...
  LD A,$0A
TXT_CTL_V_1:
  CALL NC,_SPLIT_ROW     ; CALL if at the end of text
  JP NC,TXT_CTL_V_0
  CALL CLOSE_STREAM
  CALL TXT_MEMFULL
TXT_CTL_V_2:
...
TXT_DISPLAY_A:
  ...
  LD A,H
  OR L
  JP Z,TXT_DISPLAY_A_END
  EX DE,HL
  CALL TXT_BLOCK_SEL
  POP AF
  LD B,A
  CALL TXT_SET_ROWPTR_A
  LD A,B
  PUSH AF
  JP Z,TXT_DISPLAY_A_END
  DEC A
...
TXT_CALC_COLUMN_LP:
  INC HL
  RST CPDEHL
  JP NC,TXT_CALC_COLUMN_DONE
  LD A,(HL)
  CALL TXT_ADD_GRAPH
  LD A,(HL)
  CP ' '
  JP NC,TXT_CALC_COLUMN_LP
  CP $09            ; TAB
  JP Z,TXT_CALC_COLUMN_LP
  CALL TXT_ADD_GRAPH
  JP TXT_CALC_COLUMN_LP

TXT_CALC_COLUMN_DONE:
  LD A,(TXT_COUNT)
  INC A
  LD H,A
  CALL GET_BOTTOMROW
...
IF M10
  ...
  CP $80
  JP C,DATAR_17
  CP $88                ;  LD A,256-(2*NUMLEV)?  ..TK_GOTO?
ENDIF
