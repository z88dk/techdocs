...
; z80asm -b -m -DM100 m100.asm
; (not working) z80asm -b -m -DKC85 -o KC85.BIN m100.asm
...
defc TXT_SEL_BEG   = EDTVARS+10
defc TXT_SEL_END   = EDTVARS+12
defc TXT_BUFLAG    = EDTVARS+14
defc SAVE_CSRX     = $F6EE
...
; POSIT, _TAB, ESC_L, Q_INLIN, _INLIN_BRK, _INLIN_BS, _INLIN_TAB, INXD, USING,
...
; __CLEAR, OUTC_SUB, VARPTR_A, SETFIL, __OPEN, _OPEN, CLOSE1, __MERGE, __SAVE,
...
EXEC_HL_0:
  ...
  LD (DOT),HL
  EX DE,HL
  CALL FIND_LINE		; Get first line number
  JP C,EXEC_HL_1
  POP AF
  PUSH AF
...
L0CC4:
  PUSH HL
  CALL Q_INLIN			; User interaction with question mark, HL = resulting text 
  POP BC
  JP C,L40B3
  INC HL
  LD A,(HL)
...
ESC_B:
  ...
  JP C,ESC_B_1
  INC L
; This entry point is used by the routines at ESC_X, ESC_C, ESC_A, _LF and CURS_CR.
UPD_COORDS:
  LD (CSRX),HL
  RET
...
TXT_TO_BASIC:
  ...
  LD (ERRTRP),HL
  LD HL,BLANK_BYTE
  LD D,$F8		; 'RAM' device  ($F9 on KC85 and PC8201)
  JP __MERGE_1
...
__MAX_0:
  PUSH AF
  LD HL,(HIMEM)
  LD DE,$FEF5		; -267 (same offset on MSX)
__MAX_1:
  ADD HL,DE
  DEC A
  JP P,__MAX_1
