...
; Philips VG-5000 ROM BASIC

; To build latest version (v1.1, 17/10/1984):
; z80asm -b -m -l vg5000.asm

; Older version (v1.0, 22/06/1984):  add -DV10
...
; --- Hook jump table ---
defc INTHK             = $47D0
defc CALHK             = $47D3  ; Reserved to BASIC: CALL overwrites the JP vector every time
defc SONHK             = $47D6
defc PLYHK             = $47D9
defc PRTHK             = $47DF
defc OUTHK             = $47E2  ; Used by the graphics printer driver for VW 0010 / VW 0020 printers
defc CRDHK             = $47E5
defc RINHK             = $47E8  ; a.k.a. HPINL - Hook for RINPUT/PINLIN std routine
defc INPHK             = $47EB
defc NMIHK             = $47EE  ; Handles the RESET key
defc __LPEN            = $47F1  ; add-in external function
defc __DISK            = $47F4  ; add-in external command
defc __MODEM           = $47F7  ; add-in external command

; --- BASIC system variables ---
defc INTDIV            = $47FA	; <-- IX+$00
defc INTACT            = $47FB	; <-- IX+$01
defc INTRAT            = $47FC	; <-- IX+$02
defc CURSOR            = $47FD	; <-- IX+$03, cursor mode and border color
...
; Allflg is a protection fleg, it can be set before CSAVEing something.
; It will be ORed with the cassette header at byte 13,
; so every tape block will possibly contribute in triggering the protection status.
; If ALLFLG <> 0 then:
;    SHIFT/STOP is disabled (only V1.1)
;    any direct command will scratch memory jumping at 0
;
; The user program can disable this protection at runtime by resetting ALLFLG.
defc ALLFLG            = $4889
...
IF V10
  DEFM "1.0"
ELSE
  DEFM "1.1"		; PATCH 1.1/0000/00 : Increase the version number to 1.1
ENDIF
  DEFB $00
...
; GTVLUS, TED, __LIST, PRS1, INLPNM, OUTDO_CRLF, SHIFT_STOP, RINPUT_SUB and
; QINLIN.
...
NMI:
  JP NMIHK                ; Hook entry for BASIC extensions

; NMI handler at boot time
; The NMI is executed at boot time only if an extra diode enables it.
; It switches the messages and the keyboard layout from French to International English
NMI_BOOT:
  LD A,$01
  LD (FRGFLG),A
  LD HL,KEYTAB
...
DISPLAY_1:
  ...
  OUTI                    ; send attribute
              ...
              ; PATCH 1.1/0024/00 :
              ; PATCH 1.1/0024/01 :	  Inproves screen transfer time to <40ms and improves keyboard repeat rate
...
ENDIF
  ...
  JR Z,DISPLAY_NOCURS     ; JP if cursor is hidden
...
;
; Used by the routines at _CLR_SCR, TED and CONSOLE_CLS.
CLR_SCR:
  XOR A
  CALL CLR_LINE
  LD HL,SCREEN
...
;
; Used by the routines at _SCRADR, GETCHAR, CLR_LINE, TED and L33CD.
SCRADR:
  LD C,L
  LD L,H
  LD H,$00
...
_ASCTFP_1:                                                  ;HERE TO CHECK FOR A DIGIT, A DECIMAL POINT, "E" OR "D"
  RST CHRGTB              ; Get next character              ;GET THE NEXT CHARACTER OF THE NUMBER
...
SCALMI:
  CALL P,FINMUL           ; Positive - Multiply number
  JP P,SCALMI_0           ; Positive - All done
  PUSH AF                 ; Save number of times to /10
  CALL DIV10              ; Divide by 10
  POP AF                  ; Restore count
  INC A                   ; Count divides
SCALMI_0:
  JR NZ,SCALMI            ; More to do
...
;
MULTEN:
  PUSH AF                 ; Save count                       ;SAVE EXPONENT, ENTRY FROM FOUT
  CALL MLSP10             ; Multiply number by 10            ;WE HAVE A SNG, MULTIPLY BY 10.0
  POP AF                  ; Restore count                    ;GET EXPONENT
  DEC A                   ; Count multiplies                 ;DECREASE IT
  RET
...
FINDG4:
  ...
  CALL FLOAT             ; Digit to add to FPREG

;a.k.a. PADD
FADDT:
  POP BC                 ; Restore number            ;GET PREVIOUS NUMBER OFF STACK
  POP DE
  JP FADD                ; Add BCDE to FPREG and return     ;ADD THE TWO NUMBERS

;a.k.a. EDIGIT
FINEDG:
  LD A,E                 ; Get digit                 ;EXPONENT DIGIT -- MULTIPLY EXPONENT BY 10
  RLCA                   ; Times 2                   ;FIRST BY 4
  RLCA                   ; Times 4
  ADD A,E                ; Times 5                   ;ADD 1 TO MAKE 5
  RLCA                   ; Times 10                  ;NOW DOUBLE TO GET 10
  ADD A,(HL)             ; Add next digit            ;ADD IT IN
  SUB '0'                ; Make it absolute          ;SUBTRACT OFF ASCII CODE, THE RESULT IS
                                                     ;POSITIVE ON LENGTH=2 BECAUSE OF THE ABOVE CHECK
  LD E,A                 ; Save new digit            ;STORE EXPONENT
  JR FINEC               ; Look for another digit    ;CONTINUE
...
; a.k.a. NUMPRT
		...
		;ALTERS ALL REGISTERS
;
; Used by the routines at INIT, TED, SCCPTR, __LIST and SHIFT_STOP.
LINPRT:
  LD DE,PRNUMS
  PUSH DE
...
NUMASC:
  ...
  LD (HL),' '           ; Space at start               ;PUT IN A SPACE
  JP P,SPCFST           ; Positive - Space to start    ;IF WE HAVE A NEGATIVE NUMBER, NEGATE IT
  LD (HL),'-'           ; "-" sign at start            ; AND PUT A MINUS SIGN IN THE BUFFER
SPCFST:
...
FOUTCS:
  CALL FADDH              ; Add 0.5 to FPREG      ;ROUND NUMBER TO NEAREST INTEGER
...
MAKNUM:
  ...
  PUSH AF                 ; Save count
  LD DE,POWERS_TAB        ; Powers of ten
...
__PLAY:
  CALL PLYHK              ; Hook entry for BASIC extensions
...
; Routine at 3178
__SOUND:
  CALL SONHK              ; Hook entry for BASIC extensions
  CALL GETINT
  OR A
  JR NZ,__SOUND_0
  INC A
...
; Sound output
;
; Used by the routines at _SOUND, MUSIC, TED and CONSOLE_BEL.
SOUND:
  DI
SOUND_0:
  PUSH HL
...
;
; Used by the routines at CHRGTB, INIT_CONSOLE and SHIFT_STOP.
CHR_UPDATE:
  LD B,$C0
  BIT 7,A
  JR Z,CHR_UPDATE_0
...
__TX:
  LD A,$00
  LD (IX+$0A),A           ; EXTENF - Set "Character redefinition flag" to normal characters
...
__GR:
  LD (IX+$0A),$00         ; EXTENF - Set "Character redefinition flag" to normal characters
  LD A,$80
  JP __TX_0

; Routine at 3648
__ET:
  LD (IX+$0A),$80         ; EXTENF - Set "Character redefinition flag" to redefined characters
  LD A,$00
  JP __TX_0
...
__INIT_0:
  ...
  AND $07
  LD L,A                  ; save color
  ADD A,A                 ; Set background color
  ADD A,A
  ADD A,A
  ADD A,A
  ADD A,L                 ; add ink color
  SET 7,A                 ; bit 7 must be set
...
IF V10
  JR Z,JOY_FIRE_F2_1
ELSE
  JP Z,JOY_FIRE_F2_1_V11      ; PATCH 1.1/0007/00 : Provides for 2 action button joysticks (BASIC 1.0-TB 7.1)
ENDIF
  IN A,($81)
  BIT 2,A
  JR JOY_L_R_5
...
JOY_FIRE_F2_0:      ; PATCH 1.1/0007/00 : Provides for 2 action button joysticks (BASIC 1.0-TB 7.1)
  IN A,($07)
  CPL
  AND $30
  RRCA
...
; Immediate refresh of the display, the refresh counters will be reset
;
; Routine at 3982
...
; Routine at 3992
__DISPLAY:
  LD A,32                    ;  Set to refresh the screen every 32 Interrupt occurrences
  JR Z,__DISPLAY_0
  CALL GETINT                ;  Get the custom refresh rate from the command parameter
__DISPLAY_0:
  LD (IX+$02),A              ; INTRAT
  JR __SCREEN


; Freeze the video screen for the logest interval possible (while the program runs)
; This delays the picture refresh as a visual effect or for extra speed
__STORE:
  LD (IX+$02),$00            ; INTRAT - Screen refresh rate
  LD (IX+$00),$00            ; INTDIV - Screen refresh counter
  RET
...
__CALL:
  ...
  LD (CALHK),A            ; SMC:  set opcode
  LD (CALHK+1),DE         ; SMC:  set address
  JP CALHK                ; Hook entry for BASIC extensions
...
INIT:
  ...
  LD HL,INTHK             ; Hook entries for BASIC extensions
  LD B,10*3               ; 10 entries, but we put RET everywhere
INIT_0:
  LD (HL),A
  INC HL
  DJNZ INIT_0
  ...
  LD HL,$0001
  LD (DOT),HL             ; Set cursor to screen line 0, column 1
  LD (IX+$03),$6E         ; CURSOR - reset to correct shape and enable it
  CALL INIT_CONSOLE       ; set cursor, restore video mode and defalut colors
  LD HL,ROM_RAMLOW
  LD BC,$0065
  LD DE,RAMLOW
  LDIR
  ...
  LD HL,WARMST
  LD (NMIHK+1),HL         ; Change the value of the NMI interrupt, now it will make a WARM BOOT happen when "RESET" is pressed
...
INIT_1:
  CALL PRS
  LD DE,$4241       ; look for it in RAM
  LD HL,INIT_BEL	; RET to 'READY' with startup sound
  PUSH HL
  LD HL,$6000
  LD BC,$2000
INIT_2:
...
; NMI handler at runtime, we'll get here when "RESET" is pressed
WARMST:
  PUSH AF
  IN A,($80)
  AND $40                 ; Check if CTRL is also pressed
  JR Z,WARMGO             ; Yes, do the warm reset
  POP AF
  RETN
...
WARMGO:
  ...
  EI                      ; Enable interrupts
  LD HL,$0001             ; Set cursor to screen line 0, column 1
  LD (DOT),HL
  LD (IX+$03),$6E         ; CURSOR - reset to correct shape and enable it
  CALL INIT_CONSOLE       ; set cursor, restore video mode and defalut colors
  CALL STKINI             ; reset the BASIC stack pointer
...
INIT_BEL:
  LD HL,STARTUP_SOUND     ; plays the sign-on jingle
  CALL __PLAY
  JP READY                ; re-enter BASIC at rhe "ready" point (command mode)
...
ELSE
  DEFB $F0,$F1,$F2,$F3,$F4,$F5,$F6,$F7    ; ; PATCH 1.1/0006/00 :   Correct printer translation character hex F1 (BASIC 1.0-TB 5.2)
ENDIF
...
__SAVE:
  ...
  LD HL,$0000
  LD (RETADR),HL
  LD A,$FF             ; set "output to file" flag
  LD (PRTFLG),A
  LD A,$82
  LD HL,(VARTAB)
  LD (HL),A
...
CLOAD_END:
  ...
  CALL TAPIOF
  LD HL,CASCOM
  JR C,LOAD_ABORT         ; JP if load was interrupted (STOP pressed..)
  JR NZ,CLOAD_OM_ERR
  LD A,C
  OR A
  LD A,(HL)
...
FILE_CHECKSUM:
  ...
  INC HL
  CP $83
  JR Z,FILE_CHECKSUM_1

  XOR A                   ; set "output to screen" flag to deal with possible error messages
  LD (PRTFLG),A
  CALL ENFMEM
  LD A,$FF                ; set "output to file" flag
  LD (PRTFLG),A
...
; This entry point is used by the routines at __LOAD, _CLOAD and CLOAD_HEADER.
_CASIN:
  PUSH HL
  PUSH DE
  PUSH BC
...
TAPIOF:
  CALL _TAPIOF
  LD A,(MULVAL)           ; Restore cursor status saved by TAPION
  LD (IX+$03),A           ; CURSOR
  RET
...
LOAD_BORDER:
  ...
  LD A,$29
  OUT ($8F),A             ; $29=Data register R1 + exec request
  LD A,(IX+$03)           ; pick border color
  AND 11111000b
  OR E                    ; mix it with passed value
  OUT ($CF),A
  RET
...
; a.k.a. "Transfer Address", position $2000
FNCTAB:
  DEFW __END
  DEFW __FOR
  DEFW __NEXT
...
GETCMD:
  ...
  LD A,(ALLFLG)
  OR A                     ; Any protection flag set ?
  JP NZ,$0000              ; If so, reset
...
ELSE
              ; PATCH 1.1/0004/00 :   Stops dropping out of AUTO mode on exact screen-width lines (BASIC 1.0-TB 3.2)
  JP AUTLIN_SUB
ENDIF
...
; This entry point is used by the routines at __GOTO, TED, __RESTORE, __RENUM,
...
ELSE
              ...
              ; PATCH 1.1/0009/00 :   Stops the INPUT command turning off the PAGE command (BASIC 1.0-TB 9.4)
...
IF V10
  CALL QINLIN_SUB
ELSE
              ; PATCH 1.1/0008/00 :   Corrects the multiple INPUT command data entry
              ; PATCH 1.1/0008/01 :   for second and subsequent variables
              ; PATCH 1.1/0008/02 ;                                (BASIC 1.0-TB 9.2)
...
IF V10
  ...
  CALL CONIS
ELSE
              ; PATCH 1.1/0016/00 :   Corrects the POKE command value to 
			  ;                           the positive integer range 0-255 only (BASIC 1.0-TB 16.2)
...
;
; This entry point is used by the routines at INIT, WARMST and ERROR.
STKINI:
...
__LIST_8:
  ...
  POP HL
  CCF
  RET NC                                             ;  IF PAST, THEN DONE LISTING.
...
LSTLP3:
  ...
  CP LINCON               ; Line number prefix:  LINE # CONSTANT?
  JR NZ,LSTLP4
  LD A,' '                ; Add a space BEFORE a specified line number
  RST OUTDO               ; (as a command argument)
  LD A,(HL)               ; LSB of line number
  INC HL
  EX AF,AF'
  LD A,(HL)               ; MSB of line number
  INC HL
  PUSH HL                 ; Save text pointer
  LD H,A                  ; HL=line number
  EX AF,AF'
  LD L,A
  CALL LINPRT             ; Output line number in decimal
...
LSTLP4:
  ...
  SUB TK_END-1            ; Find and output word
  LD C,A                  ; Token offset+1 to C
  LD DE,WORDS             ; Reserved word list
  CP $0F
  JR NZ,FNDTOK

  LD (IX+$08),$01         ; change video attributes
...
LISTLP:
  LD (IX+$08),$00           ; Reset video attributes         ; restore video attributes
  CALL OUTDO_CRLF
  JP LSTLP_0

; Routine at 12920
;
; Used by the routines at INIT and WARMST.
INIT_CONSOLE:
  SET 6,(IX+$03)          ; CURSOR - Show
...
; CHR$(31) - CLR-HOME
;
; Used by the routines at __INIT and CONSOLE_JPTAB.
CONSOLE_CLS:
...
; Routine at 13082
;
; Used by the routines at TED, L33CD and FINLPT.
_CHPUT:
  EXX
  PUSH AF
  LD HL,_CHPUT1
...
_CHPUT_0:
  ...
  JR NZ,_CHPUT_1          ; jump if redefined characters
...
; CHR$(16) is a specially preprogrammed character, it occupies the code $7F in the ET character set.  
; It is usually the ".." symbol, or a "diamond" during the program editing (AUTO mode)
;
; This entry point is used by the routine at CONSOLE_JPTAB.
CONSOLE_DOTDOT:
  LD A,$FF
...
; Routine at 13132
;
; Used by the routine at CONSOLE_JPTAB.
CONSOLE_I_MK:
  LD A,$0D
...
CONSOLE_POUND:
  LD A,$03
; This entry point is used by the routines at _CHPUT and CONSOLE_JPTAB.
CONSOLE_ACC_MRK:
  POP HL
  LD HL,L33CD
  PUSH HL
...
CONSOLE_JPTAB:
  ...
  JP CONSOLE_DOTDOT         ; CHR$(16) It is usually the ".." symbol
  ...
  LD A,(IX+$09)				; CHR$(31) [CLS]   ATTBAK - Background color
  JP CONSOLE_CLS
...
; 'FRE' BASIC function
;
; An undocumented feature is to return the amout of free memory above BASIC's stack
; (but within the memory size set by MEMSIZ), used for string variables.
; Just use FRE(" ")
;
; Routine at 14513
__FRE:
...
; This entry point is used by the routine at CHRGTB.
_OUTDO:
  CALL OUTHK            ; The graphics printer driver for VW 0010 / VW 0020 uses this hook code
  PUSH AF
  LD A,(PRTFLG)         ;SEE IF WE WANT TO TALK TO LPT
  OR A                  ;TEST BITS
...
FINLPT_0:
  ...
  LD (PICFLG),A
  XOR A
  LD (PRTFLG),A             ; set "output to screen" flag
  LD (PRTSTT),A
  RET
...
OUTDO_CRLF:
  ...
  LD A,(PRTFLG)           ;SEE IF WE WANT TO TALK TO LPT
  OR A                    ;TEST BITS
  JR Z,CRLF_CONSOLE       ;IF ZERO THEN NOT
  DEC A
  JR NZ,CRLF_LPT
...
SHIFT_STOP:
  IN A,($81)
  AND $01                 ; Check if STOP is pressed
  RET NZ
  IN A,($80)
  AND $04                 ; Check if SHIFT is also pressed
  RET NZ                  ; no, return
...
PROMPT_SUB:
  ...
  JP PROMPT_RET2
              ...
              ; PATCH 1.1/0019/00 :   Prevents the AUTO command from going past line number 65529 (BASIC 1.0-TB 17.2)
...
INIT_CONSOLE_SUB:
  ...
  JP  INIT_CONSOLE_1
              ...
              ; PATCH 1.1/0012/00 :
              ; PATCH 1.1/0012/01 :   Provides an optional parameter for the NEW command
...
QINLIN_5:
  JP Z,TED_CR
              ...
              ; PATCH 1.1/0023/00 :
              ; PATCH 1.1/0023/01 :	  Prevents LIST keys from operating during "ALLFLG" protection
